...
Android.prototype.startDevice = function * (caps) {
  ...
  yield this.initUiautomator(caps.permissionPatterns);
...
Android.prototype.initUiautomator = function * (permissionPatterns) {
  this.uiautomator = new UIAutomatorWD();
  this.proxy = this.uiautomator;
  permissionPatterns = permissionPatterns || '[]';
  logger.info(`checking permissionPatterns: ${permissionPatterns}`);
  yield this.uiautomator.init(this.adb, permissionPatterns);
};
