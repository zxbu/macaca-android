...
controllers.keys = function * (value) {
  ...
  if (value.endsWith('\uE00C')) {
    // 先调起键盘，后关闭
    yield this.adb.shell('adb shell ime enable com.android.adbkeyboard/.AdbIME');
    yield this.adb.shell('adb shell ime set com.android.adbkeyboard/.AdbIME');
    escape = true;
  ...
  if (escape) {
    // uirecorder传来的escape字符本意是想关闭输入法，但adbime无法用escape关闭，所以这里直接调用shell命令关闭
    yield this.adb.shell('adb shell ime disable com.android.adbkeyboard/.AdbIME');
  }
  return null;
};
