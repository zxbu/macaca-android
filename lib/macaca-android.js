...
Android.prototype.proxyCommand = function * (url, method, body) {
  ...
  return yield this.proxy.sendCommand(url, method, body);
};
...
Android.prototype.initReuse = function() {
  let reuse = parseInt(this.args.reuse, 10);
...
Android.prototype.unlock = function * () {
  ...
  if (isInstalled) {
    yield this.checkApkVersion(UnlockApk.apkPath, UnlockApk.package);
  } else {
    yield this.installApk(UnlockApk);
  }
...
Android.prototype.installApk = function * (appInfo, options = {}) {
  if (!options.permissionWatcher) {
    yield this.adb.install(appInfo.apkPath);
    return;
  }
  ...
  const installUntiSuccess = function * () {
    yield this.adb.install(appInfo.apkPath);
    const isInstalled = yield this.adb.isInstalled(appInfo.package);

    for (let i = 0; i < list.length; i++) {
      //const text = list[i];
      const res = yield this.proxy.sendCommand('/wd/hub/', 'POST');
      if (res) {
        yield this.proxy.sendCommand('/wd/hub/click', 'POST');
      }
    }
    if (!isInstalled) {
      yield installUntiSuccess();
    }
  };
  yield installUntiSuccess();
};
