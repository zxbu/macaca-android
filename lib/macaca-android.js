...
Android.prototype.stopDevice = function() {
  return _.exec('adb emu kill').then(() => {});
};
...
Android.prototype.initDevice = function *() {
  ...
  if (devices.length) {
    ...
    if (data.length === 0) {
      throw new Error('no avd created!');
    } else {
      var avdArr = data.filter(avd => {/Name:/.test(avd)}).map(avd => avd = _.trim(avd.split(':')[1]));
...
Android.prototype.getWebviews = function *() {
  ...
  if (this.isProxy()) {
    var handles = yield this.proxy.sendCommand('/session/temp/window_handles', 'GET', {});
    return handles;
  }
