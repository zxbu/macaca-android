...
Android.prototype.skipPermission = function *() {
  // execute and let continue install target app, consider make this an external file
  const skipPermissionUrl = `/wd/hub/permission/skip`;
  return yield this.proxy.sendCommand(skipPermissionUrl, 'POST', {
    'value':['继续安装', '下一步', '好', '允许', '确定', '我知道了'],
    'repeat':5});
};
